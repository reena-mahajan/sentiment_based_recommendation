...
def predict():
    if (request.method == 'POST'):
        ...
        pickled_tfidf_vectorizer = pickle.load(open('Tfidf_vectorizer_sentiment_review.pkl','rb'))
        pickled_model = pickle.load(open('Logistic_Reg_Sentiment_Review_model.pkl','rb'))
        ...
        improved_recommendations= pd.merge(recommendations,pickled_reviews_data[['prod_id','lemmatized_text']], left_on='prod_id', right_on='prod_id', how = 'left')
        test_data_for_user = pickled_tfidf_vectorizer.transform(improved_recommendations['lemmatized_text'].values.astype('U'))
